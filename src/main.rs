...
fn generate<T: Token>(input_sequence: impl Iterator<Item = T>, grammar: &Grammar<T>)
-> Vec<BinaryTree<T>> {
    generate_stacks(input_sequence, grammar)
        .filter_stacks()
        .tops()
}

fn main() {
    let x = generate("-1+2*4".chars(), &Grammar::expression());
    println!("{} trees", x.len());
    for t in x {
        println!("{}", t);
    }
    let word_sequence = vec!["the", "cat", "sat", "on", "the", "mat"];
    let x = generate(word_sequence.into_iter(), &Grammar::sentence());
    println!("{} trees", x.len());
    for t in x {
        println!("{}", t);
    }
...
mod test {
    ...
    fn test_zero_characters() {
        ...
        assert_eq!(1, x.len());
        let x = generate("".chars(), &Grammar::expression());
        assert_eq!(0, x.len());
    }
    ...
    fn test_zero_words() {
        ...
        assert_eq!(1, x.len());
        let x = generate(vec![].into_iter(), &Grammar::sentence());
        assert_eq!(0, x.len());
    }
    ...
    fn test_one_character() {
        ...
        assert_eq!(1, x.len());
        let x = generate("1".chars(), &Grammar::expression());
        assert_eq!(1, x.len());
    }
    ...
    fn test_one_word() {
        ...
        assert_eq!(1, x.len());
        let x = generate(vec!["the"].into_iter(), &Grammar::sentence());
        assert_eq!(1, x.len());
    }
    ...
    fn test_two_words() {
        ...
        assert_eq!(2, x.len(), "{x:?}");
        let x = generate(vec!["the", "cat"].into_iter(), &Grammar::sentence());
        assert_eq!(1, x.len(), "{x:?}");
    }

    #[test]
    fn test_three_characters() {
        let x = generate("1+3".chars(), &Grammar::expression());
        assert_eq!(1, x.len(), "{x:?}");
    }
    ...
    #[test]
    fn test_six_words() {
        let x = generate(vec!["the", "cat", "sat", "on", "the", "mat"].into_iter(), &Grammar::sentence());
        assert_eq!(1, x.len(), "{x:?}");
    }
}