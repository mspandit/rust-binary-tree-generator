...
use super::*;
    ...
    fn test_zero_characters() {
        ...
        let x = expression()
        .parse(& input);
        assert_eq!("[Continuation]", format!("{:?}", x));
    }
    ...
    //     assert_eq!("[Nonterminal(1), Cont(Grammar::Continuation)]", format!("{:?}", x));
    // }
    ...
    fn test_one_word() {
        ...
        assert_eq!("[Nonterminal(the), Cont(Grammar::Continuation)]", format!("{:?}", x));
    }
    ...
    fn test_two_words() {
        ...
        let x = sentence().parse(&input);
        assert_eq!("[Nonterminal(the), Nonterminal(cat), Cont(Grammar::Continuation)]", format!("{:?}", x));
    }
